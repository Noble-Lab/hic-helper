...
if __name__ == '__main__':
    ...
    import sys
    if len(sys.argv) != 8:
        print('Usage: python3 bam_4dn.py [input.bam] \
    ...
    input_bam_file = os.path.abspath(sys.argv[1])
    ...
    if resume_flag == 1 and os.path.exists(sam_pairsam_file) and os.path.getsize(sam_pairsam_file) > 1000:
        ...
        os.system('bash %s %s %s %s %s %d %s %dg ' % (pairsam_parse_sort_script_path,input_bam_file,chrom_size_file,  
    ...
    * Output: a duplicate-marked pairsam file
    ...
    if run_mode == 0:
        ...
        500000,1000000,2500000,5000000,10000000 -t %d "%(juicebox_pre_script_path,
